...
;(function(factory) {
  ...
  //初始化标题
  MobileCrop.prototype.initTitle = function(params) {
  ...
  //初始化裁剪框边框以及辅助线
  MobileCrop.prototype.initBoxBorder = function(params, onlyInit) {
  ...
  // 初始化裁剪框拖拽相关参数
  MobileCrop.prototype.initBoxCursor = function(params) {
  ...
  //初始化功能按钮
  MobileCrop.prototype.initFuncBtns = function(params) {
    /**
  ...
  //初始化旋转刻度盘
  MobileCrop.prototype.initRotateSlider = function(params) {
  ...
  //初始化相关子元素
  MobileCrop.prototype.initChilds = function() {
  ...
  MobileCrop.prototype.initBox = function(params) {
  ...
  MobileCrop.prototype.updateBox = function(params) {
    this.initBox(params)
  ...
  //获取操作点
  MobileCrop.prototype.getControlPoints = function(e) {
    if (e.touches) {
      return e.touches
    } else {
      return [
  ...
  //获取操作事件名称
  MobileCrop.prototype.getControlEvents = function() {
  ...
  //html结构
  MobileCrop.prototype.construct = function() {
    var html = ''
  ...
  //默认绘制裁剪框
  MobileCrop.prototype.defaultBorderDraw = function($cropCover) {
  ...
  //初始化
  MobileCrop.prototype.init = function() {
  ...
  //重置
  MobileCrop.prototype.reset = function() {
    this.startControl()
  ...
  //加载图片
  MobileCrop.prototype.load = function() {
    var self = this
  ...
  //设置裁剪图片
  MobileCrop.prototype.setImage = function(image) {
    if (image != null && image != '') {
      var self = this
  ...
  //显示
  MobileCrop.prototype.show = function(image) {
    if (image) {
      this.setImage(image)
    }
    this.visible = true
  ...
  //隐藏
  MobileCrop.prototype.hide = function() {
    this.visible = false
    this.$target.style.visibility = 'hidden'
  }

  //关闭
  MobileCrop.prototype.close = function() {
    this.hide()
    this.closeCallback()
  }

  //裁剪
  MobileCrop.prototype.crop = function() {
    this.getCropImage()
  ...
  //上传
  MobileCrop.prototype.uploadInput = function(evt) {
  ...
  //整角旋转
  MobileCrop.prototype.around = function() {
    this.startControl()
  ...
  // 裁剪框拖拽鼠标样式
  MobileCrop.prototype.moveCursor = function(touches) {
    var point = {
  ...
  //裁剪框移动
  MobileCrop.prototype.cropMove = function(touches) {
  ...
  //事件监听
  MobileCrop.prototype.bindEvent = function() {
    var self = this
  ...
  MobileCrop.prototype.getScaleTranslate = function(scale, scaleCenter) {
  ...
  MobileCrop.prototype.getRotateTranslate = function(angle, rotateCenter) {
  ...
  //根据图片方向计算源图片实际宽高
  MobileCrop.prototype.getRealCotentSize = function() {
  ...
  //处理图片方向
  MobileCrop.prototype.transformCoordinates = function() {
    if (this.$cropContent) {
  ...
  //获取裁剪图片
  MobileCrop.prototype.getCropImage = function() {
  ...
  //操作结束
  MobileCrop.prototype.endControl = function() {
    if (this._isControl) {
      var self = this
      this._downPoints = []
  ...
  //操作开始
  MobileCrop.prototype.startControl = function(touches) {
  ...
  //双指操作事件
  MobileCrop.prototype.isTwoFingerEvent = function(touches) {
    /**
  ...
  //内容图片移动
  MobileCrop.prototype.contentMove = function(touches) {
    var point = touches[0]
  ...
  //旋转、缩放、移动
  MobileCrop.prototype.transform = function(rotateCover, scaleKeepCover) {
    if (this.$cropContent) {
      var scaleNum = (this.scaleTimes / this.times) * this._rotateScale
      var transform = ''
      transform += ' scale(' + scaleNum + ')' //缩放
  ...
  MobileCrop.prototype.getCoverRectScale = function(outer, inner) {
    var scale = 0
  ...
  //判断 矩形A 是否完全包含 矩形B
  MobileCrop.prototype.isWholeCover = function(rectA, rectB) {
  ...
  MobileCrop.prototype.getCoverPointScale = function(point, rectPoints) {
  ...
  MobileCrop.prototype.getCoverTransform = function(transform, onlyTranslate) {
  ...
  //找出一个矩形在另一个矩形外的顶点数据
  MobileCrop.prototype.getOutDetails = function(inner, outer) {
    var outDetails = []
    for (var i = 0; i < inner.length; i++) {
      var pt = inner[i]
  ...
  //获取刚好包含某个矩形的新矩形
  MobileCrop.prototype.getCoveRect = function(rect, angle) {
    if (angle < 0) {
      angle = 90 + (angle % 90)
    } else {
      angle = angle % 90
  ...
  //计算新的变换坐标
  MobileCrop.prototype.getTransformPoints = function(transform, points) {
  ...
  //获得矩形点坐标中心
  MobileCrop.prototype.getPointsCenter = function(points) {
    var center = {
  ...
  //矩形位置形式转换为顶点坐标形式
  MobileCrop.prototype.rectToPoints = function(rect) {
    var points = []
    points.push({
  ...
  MobileCrop.prototype.getTransformMatrix = function(transform) {
  ...
  MobileCrop.prototype.getTransformFunctionName = function(transform) {
  ...
  //计算向量 a 在向量 b 上的投影向量
  MobileCrop.prototype.getProjectionVector = function(vecA, vecB) {
  ...
  MobileCrop.prototype.getPCVectorProjOnUpAndRight = function(
    point,
    rectPoints
  ) {
  ...
  MobileCrop.prototype.isPointInRectCheckByLen = function(point, rectPoints) {
    var pcv = this.getPCVectorProjOnUpAndRight(point, rectPoints)
  ...
  //根据角度和判断点是否在矩形内
  MobileCrop.prototype.isPointInRectCheckByAngle = function(point, rectPoints) {
    //先计算四个向量
    var vecs = []
  ...
  MobileCrop.prototype.getMinAngle = function(tVec, aVec) {
  ...
  //计算向量夹角
  MobileCrop.prototype.vecAngle = function(vec1, vec2) {
    var acos =
  ...
  //计算向量的模
  MobileCrop.prototype.vecLen = function(vec) {
    return Math.sqrt(vec.x * vec.x + vec.y * vec.y)
  }

  //file转image
  MobileCrop.prototype.fileToSrc = function(file, callback) {
